...
export default {
  setup() {
    ...
    async function onValidate(row) {
      state.validateName = row.name
      state.validateDisplayName = row.display_name || row.name
      state.validation = await validateTeam(row.name)
  ...
  template: `
    <div>
      ...
      <el-table :data="state.teams" v-loading="state.loading" stripe>
        <el-table-column label="队伍名" min-width="140">
          <template #default="{ row }">
            <el-link type="primary" @click="$router.push('/team/' + row.name)">{{ row.display_name || row.name }}</el-link>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="文件 ID" width="170" />
      ...
      <el-dialog v-model="state.validateVisible" :title="'校验结果：' + (state.validateDisplayName || state.validateName)" width="520px">
        <div v-if="state.validation">
          ...
          <div v-if="state.validation.errors.length" class="validate-block">
            <div v-for="e in state.validation.errors" :key="e" class="validate-error">
              <el-tag type="danger" size="small" effect="plain">错误</el-tag> {{ e }}
            </div>
          </div>
          <div v-if="state.validation.warnings.length" class="validate-block">
            <div v-for="w in state.validation.warnings" :key="w" class="validate-warn">
              <el-tag type="warning" size="small" effect="plain">警告</el-tag> {{ w }}
            </div>
          </div>
        </div>
      </el-dialog>
